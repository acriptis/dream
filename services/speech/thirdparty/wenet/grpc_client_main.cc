...
DEFINE_double(interval, 0.5, "audio sending interval");
DEFINE_double(sleep, 1.0, "sleeping time factor between sending audio chunks");
...
int main(int argc, char* argv[]) {
  ...
  // Send data every 0.5 (or given) second
  const int sample_interval = FLAGS_interval * sample_rate;
  for (int start = 0; start < num_samples; start += sample_interval) {
    ...
    std::this_thread::sleep_for(
        std::chrono::milliseconds(static_cast<int>(FLAGS_interval * 1000 * FLAGS_sleep))); // The waiting can be disabled by setting the sleep flag to 0.0.
  }
  wenet::Timer timer;
